...
function parseJsonFromUrl($url) {
    ...
    foreach ($dataArray as $dataObject) {        
        ...
        foreach ($dataObject['Пациент'] as $patient) {
            $patient = mb_strtolower($patient);
            $patientFilter = [
				"IBLOCK_ID" => 56, 
				"ACTIVE" => "Y"
			];
            $patientRes = CIBlockElement::GetList([], $patientFilter, false, false, ["ID", "NAME", "PROPERTY_B24_ID"]);
            while ($patientObj = $patientRes->Fetch()) {
                if (mb_stripos(mb_strtolower($patientObj['NAME']), $patient) !== false) {
